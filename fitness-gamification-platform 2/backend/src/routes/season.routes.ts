...
router.get('/:id/standings', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    let sql = `
      SELECT ws.*, c.name as club_name, c.city as club_city,
             ws.club_rank,
             ws.total_points,
             ws.total_checkins,
             ws.week_number as month_number,
    ...
    // Parse top_contributors JSON
    const standings = result.rows.map(row => ({
      ...row,
      top_contributors: row.top_contributors ? JSON.parse(row.top_contributors) : null
    }));
